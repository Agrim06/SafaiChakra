...
export default function App() {
  ...
  const fetchAllBins = useCallback(async () => {
    try {
      ...
      setActiveBin((prev) => (prev || uiBins.length > 0 ? prev ?? uiBins[0] : null));
      
      return allReturnedBins;
    } catch (err) {
      console.error("Failed to fetch bin list:", err);
      return [];
    }
  }, []);
    ...
    try {
      ...
      results.forEach((result, i) => {
        if (result.status === "fulfilled") {
          statusMap[bins[i]] = result.value.data;
        }
      });
  ...
  const handleOptimize = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await axios.get(`${API_BASE}/optimize-route`, {
        params: { threshold }
      });
      setRouteData(res.data);
    } catch (err) {
      setError("Routing engine unreachable. Using cached trajectory.");
    } finally {
      setOptimizing(false);
    }
  }, [threshold]);
  ...
  return (
    <div className="min-h-screen bg-[#05070a] text-white selection:bg-green-500/30 font-inter antialiased">
      ...
      <main className="max-w-[1600px] mx-auto px-6 py-8">
        ...
        {/* Error Notification */}
        {error && (
          <div className="glass-panel border-red-500/20 bg-red-500/5 mb-6 px-4 py-3 flex items-center justify-between animate-in fade-in slide-in-from-top-4 duration-500">
        ...
        {currentStatus?.is_alert && !toastHidden && (
          <div className="fixed top-24 right-6 z-[1000] glass-panel border-red-500/30 bg-red-500/10 p-4 min-w-[340px] shadow-[0_0_40px_rgba(239,68,68,0.2)] animate-in slide-in-from-right-8 duration-300">
            <div className="flex gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-[11px] text-red-200/60 mt-1 font-bold">
                  Node {activeBin} exceeded {threshold}% threshold
                </p>
                <button 
        ...
        <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 mb-10">
          <div>
            ...
            <div className="flex items-center gap-2 mt-2">
              <div className={`w-2 h-2 rounded-full ${isLive ? 'bg-green-500 animate-pulse shadow-[0_0_8px_#22c55e]' : 'bg-slate-600'}`}></div>
              <p className="text-[11px] text-slate-500 font-black uppercase tracking-[0.2em]">
                {isLive ? 'Global Grid Synchronized' : 'Grid Connection Offline'}
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
          ...
          <section className="xl:col-span-4 space-y-8">
            <BinCard status={currentStatus} loading={loading} threshold={threshold} />
            <ControlPanel
              onRefresh={fetchData}
              ...
              statuses={statuses}
            />
            <AgentPanel route={routeData?.route} optimizing={optimizing} status={currentStatus} />
          </section>
